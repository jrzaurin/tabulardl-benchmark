...
RAW_DATA_DIR = ROOT_DIR / "raw_data/bank_marketing_kaggle"
PROCESSED_DATA_DIR = ROOT_DIR / "processed_data/bank_marketing_kaggle"

if not os.path.isdir(PROCESSED_DATA_DIR):
    os.makedirs(PROCESSED_DATA_DIR)

bankm = pd.read_csv(RAW_DATA_DIR / "bank.csv")
bankm.drop("duration", axis=1, inplace=True)

bankm["target"] = (bankm["deposit"].apply(lambda x: x == "yes")).astype(int)
bankm.drop("deposit", axis=1, inplace=True)
