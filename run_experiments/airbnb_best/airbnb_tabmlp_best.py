...
def set_dirs(model_name):
    ...
    models_dir = WORKDIR / "/".join(["best_models", "airbnb", model_name])
    results_dir = WORKDIR / "/".join(["results", "airbnb", model_name])
...
def load_dataset():
    ...
    drop_cols = ["id", "host_id", "host_since", "latitude", "longitude", "description"]
...
def prepare_data(results_dir, exp_idx=0):
    ...
    args = read_best_model_args(results_dir, exp_idx=exp_idx)
...
if __name__ == "__main__":
    ...
    args, prepare_tab, X_train, X_test, y_train, y_test = prepare_data(
        results_dir, exp_idx=1
    )
