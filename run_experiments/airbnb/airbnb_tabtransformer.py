...
from pytorch_widedeep.models import DeepText, TabTransformer, Wide, WideDeep
...
RESULTS_DIR = WORKDIR / "results/airbnb/tabtransformer"
...
num_cols = [
    ...
    if c not in cat_embed_cols + wide_cols + ["target", "description"]
]
...
prepare_tab = TabPreprocessor(
    embed_cols=cat_embed_cols, continuous_cols=num_cols, for_tabtransformer=True
)
...
if args.save_results:
    ...
    filename = "_".join(["airbnb_tabtransformer", suffix]) + ".p"
    results_d = {}
