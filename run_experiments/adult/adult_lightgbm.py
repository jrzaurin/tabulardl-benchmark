...
RESULTS_DIR = WORKDIR / "results/adult/lightgbm"
if not RESULTS_DIR.is_dir():
    os.makedirs(RESULTS_DIR)

MODELS_DIR = WORKDIR / "models/adult/lightgbm"
...
lgbvalid = lgbDataset(
    valid_le[cat_cols], valid_le.target, reference=lgbtrain, free_raw_data=False
)
...
lgbtest = lgbDataset(
    test_le[cat_cols], test_le.target, reference=flgbtrain, free_raw_data=False
)
...
results_filename = "_".join(["adult_lightgbm", suffix]) + ".p"
results_d = {}
...
model_filename = "_".join(["model_adult_lightgbm", suffix]) + ".p"
